...
class Solution {
    ...
    var queue = PriorityQueue<TaskData>.init { (a, b) -> Bool in
...
//        }
        for i in 0..<tasks.count {
            if queue.isEmpty {
                ...
                currentTime = max(minEnqueueTime,currentTime)
            }
            
            while sortedEnqueueTime.first ?? Int.max <= currentTime {
//                print("SET:\(sortedEnqueueTime.first) CT:\(currentTime)")
//                print("ENQUEUE:\(tasksByEnqueueTime[sortedEnqueueTime.first!]!)")
                tasksByEnqueueTime[sortedEnqueueTime.first!]?.forEach({ (task) in
                    queue.enqueue(task)
                })
                sortedEnqueueTime.removeFirst()
            }
...
//            print(queue.dequeue()!.index)
        }
        return result
    }
}
let result = Solution().getOrder([[1,2],[2,4],[3,2],[4,1]])
